...
# X * 0 = 0
mul_zero_rule = ASTQuicheTree.make_rule("__quiche__x * 0", "0")

# X / 1 = X
div_one_rule = ASTQuicheTree.make_rule("__quiche__x / 1", "__quiche__x")

# X / X = 1
div_self_rule = ASTQuicheTree.make_conditional_rule(
    "__quiche__x / __quiche__x",
    "1",
    lambda eg, eid, env: not any(
        [
            (not enode.args)
            and enode.key in [("int", Constant, 0, None), ("int", Num, 0)]
            for enode in eg.lookup_eclass(eg.env_lookup(env, "__quiche__x"))
        ]
    ),
)

# X * 1 = X
mul_one_rule = ASTQuicheTree.make_rule("__quiche__x * 1", "__quiche__x")

# X + 0 = X
add_zero_rule = ASTQuicheTree.make_rule("__quiche__x + 0", "__quiche__x")

# X - 0 = X
sub_zero_rule = ASTQuicheTree.make_rule("__quiche__x - 0", "__quiche__x")

# X - X = 0
sub_self_rule = ASTQuicheTree.make_rule("__quiche__x - __quiche__x", "0")

# X * Y = Y * X
mul_commutativity_rule = ASTQuicheTree.make_rule(
    "__quiche__x * __quiche__y", "__quiche__y * __quiche__x"
)
...
def get_all_arith_rules():
    ...
    double_rules = [
        mul_distributivity_over_add_rules,
        mul_distributivity_over_sub_rules,
        add_associativity_rules,
        sub_add_ordering_rules,
        sub_sub_ordering_rules,
        add_sub_ordering_rules,
        mul_associativity_rules,
        mul_div_ordering_rules,
    ]
